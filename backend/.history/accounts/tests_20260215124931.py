...
class OTP(models.Model):
    ...
    email = models.EmailField(
        verbose_name='Email Address',
        db_index=True,
        help_text='Email address for OTP verification'
    )
    ...
    def save(self, *args, **kwargs):
        """
        Override save to set OTP code and expiration time if not set.
        """
        import random
        from datetime import timedelta
        from django.utils import timezone
...
class ProfileLinkTest(TestCase):
    def setUp(self):
        self.doctor = User.objects.create_user(
            email="doctor@carenova.com",
            password="docpassword",
            user_type="doctor"
        )

    def test_doctor_profile_creation(self):
        """Verify DoctorProfile links correctly to a Doctor user."""
        doc_profile = DoctorProfile.objects.create(
            user=self.doctor,
            license_number="MED-12345",
            specialization="Cardiology"
        )
        self.assertEqual(self.doctor.doctor_profile.license_number, "MED-12345")
