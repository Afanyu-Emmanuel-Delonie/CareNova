...
class CareNovaIntegrationTest(APITestCase):
    ...
    def test_complete_medical_to_purchase_flow(self):
        # STEP A: Patient tries to buy medicine WITHOUT prescription (Should Fail)
        ...
        # STEP B: Patient uploads a lab result
        lab = LabResult.objects.create(patient=self.patient, title="Blood Test")

        # STEP C: Doctor provides diagnosis and prescription
        ...
        # Note: Medicine name in prescription must match product name for our validation logic
        ...
        # STEP D: Patient tries to buy medicine WITH prescription (Should Succeed)
        self.client.force_authenticate(user=self.patient)
        response = self.client.post(reverse('checkout'), order_data, format='json')
        
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
        self.patient.refresh_from_db()
        self.assertEqual(self.patient.balance, Decimal("50.00")) # 100 - 50 = 50
        
        # STEP E: Verify Inventory was reduced
