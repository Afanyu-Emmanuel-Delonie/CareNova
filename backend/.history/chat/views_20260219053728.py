...
from .serializers import (
    AppointmentSerializer, ReviewSerializer, AvailabilitySlotSerializer,
    BulkSlotSerializer, DoctorAgendaSerializer, CompleteAppointmentSerializer,
    PatientHistorySerializer, RecurrentAppointmentSerializer,
)
...
class AppointmentViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        # FIX: Guard against AnonymousUser during drf-spectacular schema introspection.
    ...
    @extend_schema(
        ...
        description=(
            "Allows the assigned doctor to mark a CONFIRMED appointment as COMPLETED and attach "
            "a clinical note and prescriptions. Once completed, the patient becomes eligible "
            "to submit a review."
        ),
        request=CompleteAppointmentSerializer,
        responses={
            200: OpenApiResponse(description="Appointment marked as completed. Patient can now leave a review."),
            400: OpenApiResponse(description="Appointment is not in CONFIRMED status, or invalid payload."),
            403: OpenApiResponse(description="Only the assigned doctor can complete this appointment."),
        },
        tags=["Appointments"],
    )
    ...
    @extend_schema(
        summary="Book a recurring appointment series",
        description=(
            "Patient-only endpoint. Creates a series of recurring appointments with a single doctor "
            "across multiple calculated dates. All appointments share a recurrence_group_id so they "
            "can be managed together. Matching AvailabilitySlots are automatically marked as booked. "
            "Dates must be pre-calculated and provided via RecurrentAppointmentSerializer."
        ),
        request=RecurrentAppointmentSerializer,
        responses={
            201: OpenApiResponse(
                description=(
                    "Series created. Returns total count and the shared recurrence_group_id "
                    "for future management of the series."
                )
            ),
            400: OpenApiResponse(description="Validation error — invalid payload or no slots available for one or more dates."),
            403: OpenApiResponse(description="Only patients can book recurring appointments."),
        },
        tags=["Appointments"],
    )
    @action(detail=False, methods=['post'], url_path='book-recurrent')
    def book_recurrent(self, request):
        serializer = RecurrentAppointmentSerializer(data=request.data, context={'request': request})
        ...
        if serializer.is_valid():
            ...
            dates = data['calculated_dates']

            appointments_to_create = []
            for index, appointment_date in enumerate(dates):
                appointments_to_create.append(
                    Appointment(
                        patient=request.user.profile.patient_data,
                        doctor_id=doctor_id,
            ...
            created_list = Appointment.objects.bulk_create(appointments_to_create)

            # Mark matching slots as booked so they disappear from patient search
            ...
            return Response(
                {
                    "message": "Series of appointments created successfully.",
                    "total_count": len(created_list),
                    "recurrence_group_id": group_id,
                },
    ...
    @extend_schema(
        ...
        description=(
            "Cancels all future PENDING or CONFIRMED appointments in the same recurrence group "
            "as the specified appointment. Only the patient who owns the series or the assigned "
            "doctor may cancel it. Cancelled slots are automatically released back to availability "
            "so other patients can book those times."
        ),
        request=None,
        responses={
            200: OpenApiResponse(
                description="Series cancelled. Returns the count of appointments cancelled and slots released."
            ),
            400: OpenApiResponse(description="Appointment is not part of a recurring series."),
            403: OpenApiResponse(description="Only the owning patient or assigned doctor can cancel this series."),
        },
        tags=["Appointments"],
    )
    ...
    def cancel_series(self, request, pk=None):
        """
        Cancels all future appointments belonging to the same recurrence group.
        """
        current_appointment = self.get_object()

        if not current_appointment.is_recurring or not current_appointment.recurrence_group_id:
            return Response({"error": "This is not part of a recurring series."}, status=400)

        # Security: only the owning patient or the assigned doctor can cancel
        user_profile = request.user.profile
        if not (user_profile.patient_data == current_appointment.patient or
                user_profile.doctor_data == current_appointment.doctor):
            return Response({"error": "Unauthorized to cancel this series."}, status=403)

        with transaction.atomic():
            series = Appointment.objects.filter(
                recurrence_group_id=current_appointment.recurrence_group_id,
                status__in=['PENDING', 'CONFIRMED'],
                appointment_date__gte=timezone.now().date(),
            )

            # Capture the dates BEFORE the bulk update wipes the queryset results.
            # If collected after .update(), the filter returns nothing and no slots
            # are released — the original bug this fix addresses.
            dates_to_release = list(series.values_list('appointment_date', flat=True))
            count = len(dates_to_release)

            series.update(status='CANCELLED')

            AvailabilitySlot.objects.filter(
                doctor=current_appointment.doctor,
                date__in=dates_to_release,
                start_time=current_appointment.appointment_time,
            ).update(is_booked=False)

        return Response(
            {"message": f"Successfully cancelled {count} appointments in this series and released their time slots."},
            status=status.HTTP_200_OK,
        )
...
@extend_schema_view(
    list=extend_schema(
        summary="List all availability slots (bulk view)",
        description="Returns the full list of availability slots. Intended for admin and doctor bulk management.",
        responses={200: BulkSlotSerializer(many=True)},
        tags=["Availability"],
    ),
    retrieve=extend_schema(
        summary="Retrieve a single availability slot (bulk view)",
        responses={
            200: BulkSlotSerializer,
            404: OpenApiResponse(description="Slot not found."),
        },
        tags=["Availability"],
    ),
    create=extend_schema(
        summary="Create a single availability slot (bulk view)",
        request=BulkSlotSerializer,
        responses={
            201: BulkSlotSerializer,
            400: OpenApiResponse(description="Validation error."),
        },
        tags=["Availability"],
    ),
    update=extend_schema(
        summary="Update an availability slot (bulk view)",
        request=BulkSlotSerializer,
        responses={
            200: BulkSlotSerializer,
            400: OpenApiResponse(description="Validation error."),
    ...
    partial_update=extend_schema(
        summary="Partially update an availability slot (bulk view)",
        request=BulkSlotSerializer,
        responses={
            200: BulkSlotSerializer,
            400: OpenApiResponse(description="Validation error."),
    ...
    destroy=extend_schema(
        summary="Delete an availability slot (bulk view)",
        responses={
            204: OpenApiResponse(description="Slot deleted."),
            404: OpenApiResponse(description="Slot not found."),
        },
...
@extend_schema_view(
    list=extend_schema(
        summary="List patient's completed appointment history",
        description=(
            "Returns a chronological list of all COMPLETED appointments for the authenticated patient, "
            "ordered by most recent first. Only the patient's own history is accessible."
        ),
        responses={200: PatientHistorySerializer(many=True)},
        tags=["Patient History"],
    ),
    retrieve=extend_schema(
        summary="Retrieve a completed appointment from history",
        description="Returns the full details of a single completed appointment from the patient's history.",
        responses={
            200: PatientHistorySerializer,
            404: OpenApiResponse(description="Appointment not found in patient history."),
        },
        tags=["Patient History"],
    ),
)
