...
from .serializers import AppointmentSerializer, ReviewSerializer, AvailabilitySlotSerializer, BulkSlotSerializer, DoctorAgendaSerializer
...
class DoctorAgendaViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    def get_queryset(self):
        doctor = self.request.sers.doctor_profile
        data_str = self.request.query_param.get('data')
        if data_str:
            target_data = data_str
        else:
            target_data = timezone.now().date()
