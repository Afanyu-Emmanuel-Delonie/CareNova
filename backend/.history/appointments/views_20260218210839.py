...
class AppointmentViewSet(viewsets.ModelViewSet):
    ...
    def patient_cancel(self, request, pk=None):
        ...
        # Security: Ensure only the patient who owns this can cancel it
    ...
    def admin_traffic_monitor(self, request):
        ...
        # 1. Critical: Emergencies that are still PENDING
        unattended_emergencies = Appointment.objects.filter(
            is_emergency=True, 
            status='PENDING'
        )

        # 2. Critical: Appointments cancelled by patients because of doctor delays
        rerouted_cases = Appointment.objects.filter(
            status='CANCELLED',
            reason__icontains="delay" # Or a specific code you set
        )
