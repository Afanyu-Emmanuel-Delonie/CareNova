...
class AppointmentViewSet(viewsets.ModelViewSet):
    ...
    @extend_schema(
        ...
        description=(
            "Doctor-only endpoint to mark a confirmed appointment as completed after "
            "submitting required clinical notes and optional prescriptions."
        ),
        request=CompleteAppointmentSerializer,
        responses={
            200: OpenApiResponse(description="Appointment marked as completed."),
            400: OpenApiResponse(description="Invalid payload or appointment is not confirmed."),
    ...
    def complete_appointment(self, request, pk=None):
        ...
        serializer = CompleteAppointmentSerializer(appointment, data=request.data, partial=True)
        if serializer.is_valid():
            serializer.save(status=Appointment.Status.COMPLETED)
            return Response(
                {"message": "Appointment marked as completed. Patient can now leave a review."},
                status=status.HTTP_200_OK,
            )
...
class DoctorAgendaViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    def get_queryset(self):
        ...
        doctor = user.profile.doctor_data
...
class PatientHistoryViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    def get_queryset(self):
        ...
        patient = self.request.user.profile.patient_data
        return Appointment.objects.filter(
            patient=patient,
            status='COMPLETED'
